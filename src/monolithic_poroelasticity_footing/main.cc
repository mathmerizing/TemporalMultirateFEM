 ...
 * ==========================================
 * Tensor-product space-time code for a 3+1D Footing problem with Q^s finite elements in space and dG-Q^r finite elements in time: cG(s)dG(r)
...
// For the Footing problem we have DIM = 3
#ifndef DIM
#  define DIM 3
#endif
...
double traction_y_biot = 0.0;
double traction_z_biot = -1.0e+7;
...
template<>
void SpaceTime<3>::print_grids(std::string file_name_space, std::string file_name_time_displacement, std::string file_name_time_pressure, std::string file_name_time_joint) {

	//////////
	// space
	//

	// NOTE: DO NOT PRINT 3D SPATIAL GRID
...
void SpaceTime<3>::make_grids() {
	...
	// create 3D rectangular spatial grid with 8x8x8 elements and vertices (-32,-32,0) and (32,32,64)
	GridGenerator::hyper_rectangle(space_triangulation, Point<3>(-32.0, -32.0, 0.0), Point<3>(32.0, 32.0, 64.0));
	space_triangulation.refine_global(3);
	// space_triangulation.refine_global(2); // for debugging
	// space_triangulation.refine_global(1); // for debugging

	// mark boundaries
	for (auto &cell : space_triangulation.cell_iterators())
		for (unsigned int face = 0; face < GeometryInfo<3>::faces_per_cell; face++)
			if (cell->face(face)->at_boundary()) {
				if (cell->face(face)->center()[2] < 0. + 1e-10)
					cell->face(face)->set_boundary_id(2);
				else if (cell->face(face)->center()[2] > 64. - 1e-10 && cell->face(face)->center()[0] < 16. + 1e-10 && cell->face(face)->center()[0] > -16. - 1e-10 && cell->face(face)->center()[1] < 16. +  1e-10 && cell->face(face)->center()[1] > -16. -  1e-10)
...
// 				}
			...
			for (const unsigned int space_face : space_cell->face_indices())
				if (space_cell->at_boundary(space_face) && (space_cell->face(space_face)->boundary_id() == 1)) // face is at top compression boundary
				{
...
// #endif
			...
			for (const unsigned int space_face : space_cell->face_indices())
				if (space_cell->at_boundary(space_face) && (space_cell->face(space_face)->boundary_id() == 1)) // face is at top boundary
				{
					space_fe_face_values.reinit(space_cell, space_face);
					for (const auto &time_cell : time_dof_handler->active_cell_iterators()) {
						time_fe_values.reinit(time_cell);
...
void SpaceTime<dim>::apply_boundary_conditions(std::shared_ptr<Slab> &slab) {
	// walls with boundary ids:
	// 1: compression (part of top)
	// 2: bottom
	// 3: remaining boundaries

	// boundary conditions are given as:
	// 1: σ(u)·n = -0.1 e_z (inhomogeneous Neumann on top with -16 <= x,y <= 16)
	// 2:      p = 0        (homogeneous Dirichlet on bottom)
	// 2:      u = (0,0,0)  (homogeneous Dirichlet on bottom)
	// 3: σ(u)·n = (0,0,0) 	(homogeneous Neumann everywhere else)
   ...
   //
   {
	   ...
	   for (const auto &time_cell : slab->displacement_time_dof_handler.active_cell_iterators())
	   {
			...
			for (const unsigned int qq : time_fe_values.quadrature_point_indices())
			{
				...
				// 2: u = (0,0,0) (Dirichlet displacement;   bottom)
				VectorTools::interpolate_boundary_values(space_dof_handler, 2, boundary_func, boundary_values, ComponentMask({true, true, true, false}));
   ...
   //
   {
	   ...
	   for (const auto &time_cell : slab->pressure_time_dof_handler.active_cell_iterators())
	   {
			...
			for (const unsigned int qq : time_fe_values.quadrature_point_indices())
			{
				...
				// determine spatial boundary values at temporal support point
				std::map<types::global_dof_index, double> boundary_values;
				// 1:   p = 0 (Dirichlet pressure;      bottom)
				VectorTools::interpolate_boundary_values(space_dof_handler, 1, boundary_func, boundary_values, ComponentMask({false, false, false, true}));
...
template<>
void SpaceTime<3>::output_results(
        std::shared_ptr<Slab> &slab, const unsigned int refinement_cycle, unsigned int slab_number, bool last_slab) {
        std::string output_dir = "output/dim=3/cycle=" + std::to_string(refinement_cycle) + "/";
		...
		for (unsigned int i = 0; i < solution_at_t_qq.size(); i++)
		{
			...
			std::vector<DataComponentInterpretation::DataComponentInterpretation> data_component_interpretation(3+1, DataComponentInterpretation::component_is_scalar);
			data_out.attach_dof_handler(space_dof_handler);  
			data_out.add_data_vector(solution_at_t_qq[i], solution_names, DataOut<3>::type_dof_data, data_component_interpretation);
...
void SpaceTime<dim>::run() {
	...
	for (unsigned int cycle = 0; cycle < max_n_refinement_cycles; ++cycle) {
		...
		// Renumber spatials DoFs into displacement, pressure
  		DoFRenumbering::component_wise(space_dof_handler, {0, 0, 0, 1});

		// number of space DoFs per component
		const std::vector<types::global_dof_index> dofs_per_block = DoFTools::count_dofs_per_fe_block(space_dof_handler, {0, 0, 0, 1});
...
// 			process_solution(slabs[k], cycle, (k == slabs.size()-1));
	...
	double reference_value = 1.207537739863780e+14; // N = 50,000; pressure_ref = 0; displacement_ref = 0
...
int main(int argc, char** argv) {
	try {
		...
		double T = 5.0e+6;
		int N = 125;
		// int N = 50000; // reference calculation
		double dt = T / N;
		...
		SpaceTime<3> space_time_problem(
